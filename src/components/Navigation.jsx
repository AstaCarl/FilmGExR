...
export default function Navigation({ navigationData }) {
  ...
  return (
    <header className={`page-content-container fixed top-0 z-[1] flex justify-between w-full pt-4 transition-all duration-300 ease-in-out${
      isOpen
        ? "z-[999] bg-off-white"
        : "bg-transparent"
    }`}>
      ...
      <div className={`flex flex-col items-end pt-4 ${isOpen ? "justify-between h-screen" : "justify-center"}`}>
        ...
        isOpen
          ? "translate-y-[4px] rotate-45 bg-red"
          : " translate-x-0 rotate-0"
      }`}></div>
            <div className= {`h-0.5 bg-black w-6 rounded-full ${
        isOpen
          ? "opacity-0"
          : "opacity-1"
      }`}></div>
          <div className= {`h-0.5 bg-black w-6 rounded-full transition-transform duration-300 ease-linear ${
        isOpen
          ? "-translate-y-[8px] -rotate-45 bg-red"
          : " translate-x-0 rotate-0"
      }`}></div>
          </button>
          <nav className={`transition-all delay-100 duration-700 ease-in-out flex flex-col pb-10 items-end justify-end md:h-full md:justify-center ${isOpen ? 'translate-x-0 h-screen' : 'translate-x-full opacity-0 h-0 overflow-hidden md:translate-x-0 md:opacity-100'}`}>
